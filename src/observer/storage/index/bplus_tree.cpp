...
{
  std::vector<AttrType> attr{attr_type};
  std::vector<int> len{attr_length};
  return create(file_name,attr,len,internal_max_size,leaf_max_size);
}
RC BplusTreeHandler::create(const char *file_name, std::vector<AttrType>& attr_type, std::vector<int>& attr_length, int internal_max_size /* = -1*/,
    int leaf_max_size /* = -1 */)
{
  ...
  int sum_len=0;
  for(int i:attr_length)
    sum_len+=i;

  if (internal_max_size < 0) { //sum attr_len
    internal_max_size = calc_internal_page_capacity(sum_len);
  }
  if (leaf_max_size < 0) {
    leaf_max_size = calc_leaf_page_capacity(sum_len);
  }

  char *pdata = header_frame->data();
  IndexFileHeader *file_header = (IndexFileHeader *)pdata;
  if (attr_length.size() > MAX_COLUMN_NUM) {
    // TODO: modify error code
    LOG_WARN("multi index's column num exceeds the max column num");
    return RC::INVALID_ARGUMENT;
  }
  file_header->column_num = attr_length.size();
  for (int i = 0; i < attr_length.size(); ++i) {
    file_header->attr_length[i] = attr_length[i];
    file_header->attr_type[i] = attr_type[i];
  }
  file_header->key_length = sum_len + sizeof(RID);
  ...
  key_comparator_.init(attr_type, attr_length); //to do
...
RC BplusTreeHandler::open(const char *file_name)
{
  ...
  disk_buffer_pool->unpin_page(frame);
  std::vector<AttrType> attr_types(file_header_.column_num);
  std::vector<int> attr_lengths(file_header_.column_num);
  for (int i = 0; i < attr_types.size(); ++i) {
    attr_types[i] = file_header_.attr_type[i];
    attr_lengths[i] = file_header_.attr_length[i];
  }
...
MemPoolItem::unique_ptr BplusTreeHandler::make_key(const std::vector<const char *> &user_key, const RID &rid)
{
  ...
  assert(user_key.size() == file_header_.column_num);
  int offset = 0;
...
//RC BplusTreeHandler::delete_entry(const char *user_key, const RID *rid)
RC BplusTreeHandler::delete_entry(std::vector<const char *> &user_key, const RID *rid) 
{
  ...
  int offset=0;
  for(int i=0;i<user_key.size();i++) {
    memcpy(key+offset, user_key[i], file_header_.attr_length[i]);
    offset += file_header_.attr_length[i];
  }
    ...
    const char *user_key, int key_len, bool want_greater, char **fixed_key, bool *should_inclusive)
{
  ...
  *should_inclusive = false;
  int sum_len=0;
  for(auto x:tree_handler_.file_header_.attr_length)
    sum_len+=x;
  int32_t attr_length = sum_len;
